...
function pintarColores(p){
  ...
  if (!paleta.colores.length){
    caja.innerHTML = '<h2>Color customization</h2><p class="paleta-nota">' +
  ...
  caja.innerHTML =
    ...
    '<div class="paleta">' +
      ...
      // El "+" indica que la carta no se agota aquí
      '<span class="muestra-mas" title="Consulta otros tonos con tu asesor">+</span>' +
    '</div>' +
...
function pintarTecnicas(p){
  ...
  caja.innerHTML =
    '<h2>Recommended branding techniques</h2>' +
    '<div class="tecnicas">' +
      p.tecnicas.map(t => {
...
function pintarDatos(p){
  const filas = [
    ["Minimum order", p.minimo + " units"],
    ["Category", nombreCategoria(p.categoria)]
  ];
  if (p.tela) filas.push(["Fabric", p.tela]);

  $("#bloque-datos").innerHTML =
    '<h2>Product details</h2><div class="datos">' +
      filas.map(([k, v]) =>
...
function pintarMedidasSimples(p){
  ...
  caja.innerHTML =
    '<h2>Measurements</h2><div class="tabla-envoltura"><table class="tallas">' +
      '<thead><tr><th scope="col">Size</th><th scope="col">Measurement</th></tr></thead><tbody>' +
      p.medidas.map(m =>
...
function pintarSeo(p){
  const titulo = p.nombre + " — custom corporate apparel | " + window.CONFIG.marca;
  document.title = titulo;

  const meta = (sel, valor) => { const el = $(sel); if (el) el.setAttribute("content", valor); };
  meta('meta[name="description"]', p.resumen + " From " + p.minimo + " units, branded with your logo.");
  ...
  function render(){
    ...
    caja.innerHTML =
      '<a class="btn btn-primario btn-bloque" href="' + waProducto(p.nombre) + '" target="_blank" rel="noopener">' +
        ICONOS.whatsapp + 'Talk to a consultant</a>' +
      '<button type="button" class="btn btn-bloque ' + (dentro ? 'btn-oscuro' : 'btn-linea') + '" id="btn-cotizar">' +
        (dentro ? ICONOS.check + 'Added to your quote' : ICONOS.mas + 'Add to my quote') +
      '</button>';
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!p){
    $("#ficha").innerHTML =
      '<div class="vacio-resultado" style="grid-column:1/-1">' +
        '<h3>We could not find that item</h3>' +
        '<p>The link may be out of date.</p>' +
        '<a class="btn btn-oscuro" href="index.html">See the full catalog</a>' +
      '</div>';
    montarCabecera();
    return;
  }
  ...
  if (det){
    ...
    det.classList.toggle("oculto", !p.detalle);
  }
  ...
  if (p.medidas)                 pintarMedidasSimples(p);
    ...
    cajaTallas.innerHTML =
      '<h2>Size guide</h2>' +
      '<p class="paleta-nota">The size chart for this item is being reviewed. ' +
      'Write to us and we will send it right away.</p>';
  }
});

...
function pintarRejilla(){
  ...
  rejilla.innerHTML = lista.length
    ...
    : '<div class="vacio-resultado">' +
        ...
        '<p>Clear a filter, or write to us: we also manufacture from your own design.</p>' +
        '<a class="btn btn-primario" href="' + waGeneral() + '" target="_blank" rel="noopener">' +
          ICONOS.whatsapp + 'Talk to a consultant</a>' +
      '</div>';

  const conteo = $("#conteo");
  if (conteo) conteo.textContent = lista.length + (lista.length === 1 ? " item" : " items");
...
function pintarFiltros(){
  ...
  $("#filtro-categorias").innerHTML =
    '<button type="button" class="pastilla" data-categoria="todas" aria-pressed="' +
      (estado.categoria === "todas") + '">All</button>' +

/* ==========================================================================
   ...
   Menu, search, quote tray, WhatsApp links and SVG diagrams.
...
function etiquetaFoto(p, n){
  const vistas = ["front view", "branding detail", "back view"];
  return p.nombre + " custom branded — " + (vistas[n-1] || "view " + n);
}
...
function fotoConRespaldo(id, n, alt, clase){
  return imagenFlexible("../img/productos/" + id + "-" + n, alt, clase, "sin-foto", "Photo " + n + " coming soon");
}
    ...
    barra.innerHTML =
      '<div class="contenedor"><div class="bandeja-fila">' +
        '<div class="bandeja-texto">' +
          '<strong>' + n + (n === 1 ? " item selected" : " items selected") + '</strong>' +
          '<span>Send them all in a single message</span>' +
        '</div>' +
        '<div class="bandeja-items">' +
          this.items.map(id => {
            const p = producto(id);
            return '<span class="chip">' + escapar(p ? p.nombre : id) +
                   '<button type="button" data-quitar="' + id + '" aria-label="Remove ' +
                   escapar(p ? p.nombre : id) + '">×</button></span>';
          }).join("") +
        '</div>' +
        '<a class="btn btn-primario" href="' + enlaceWhatsapp(this.mensaje()) + '" target="_blank" rel="noopener">' +
          ICONOS.whatsapp + 'Request a quote</a>' +
        '<button type="button" class="btn btn-linea-claro btn-chico" id="vaciar-bandeja">Clear</button>' +
      '</div></div>';
...
/* Cada medida se muestra en pulgadas y, entre paréntesis, en centímetros */
function pulgadas(cm){
  const n = parseFloat(cm);
  if (isNaN(n)) return cm;
  return (Math.round(n / 2.54 * 10) / 10) + ' in <span class="cm">(' + cm + ' cm)</span>';
}
...
function dibujarTallas(mapa, contenedor){
  ...
  const etiqueta = { hombre:"Men", mujer:"Women", unisex:"Unisex" };
  ...
  contenedor.innerHTML =
    '<h2>Size guide</h2>' + pestanas + '<div id="tallas-cuerpo"></div>' +
      ...
      '<div class="tallas-caja' + (conImagen ? '' : ' tallas-caja-sola') + '">' +
        (conImagen
          ? '<figure class="tallas-esquema" data-guia="' + clave + '">' +
              '<img src="../img/tallas/' + clave + '.jpg" alt="Size guide"' +
              ' loading="lazy" decoding="async"' +
              ' onerror="ocultarGuia(this)">' +
              '<figcaption>Tap to enlarge</figcaption>' +
            '</figure>'
          : '') +
        '<div class="tabla-envoltura"><table class="tallas">' +
          ...
          Object.entries(t.filas).map(([talla, medidas]) =>
            ...
            medidas.map(m => '<td>' + pulgadas(m) + '</td>').join("") + '</tr>'
          ).join("") +
        '</tbody></table></div>' +
      '</div>';
  }
...
function activarGuiaTallas(contenedor){
  ...
  fig.addEventListener("click", () => {
    ...
    if (actual) Visor.abrir([actual.getAttribute("src")], "Size guide", 0);
  });
}
...
function montarVideo(){
  ...
  mando.innerHTML =
    '<button type="button" data-video="pausa" aria-label="Pause video">' + ICONOS.pausa + '</button>' +
    '<button type="button" data-video="sonido" aria-label="Turn sound on">' + ICONOS.mudo + '</button>';
  ...
  bPausa.addEventListener("click", () => {
    if (video.paused){
      ...
      bPausa.setAttribute("aria-label", "Pause video");
    } else {
      video.pause();
      bPausa.innerHTML = ICONOS.play;
      bPausa.setAttribute("aria-label", "Resume video");
    }
  });
  ...
  bSonido.addEventListener("click", () => {
    ...
    bSonido.setAttribute("aria-label", video.muted ? "Turn sound on" : "Mute");
  });
  ...
  function reproducir(){
    ...
    if (intento && intento.catch) intento.catch(() => {
      bPausa.innerHTML = ICONOS.play;
      bPausa.setAttribute("aria-label", "Play video");
    });
  }
...
function mando(){
  return '<div class="cinta-mando">' +
    '<button type="button" data-cinta="atras" aria-label="Previous brands">' + ICONOS.izq + '</button>' +
    '<button type="button" data-cinta="pausa" aria-label="Pause motion">' + ICONOS.pausa + '</button>' +
    '<button type="button" data-cinta="adelante" aria-label="Next brands">' + ICONOS.der + '</button>' +
  '</div>';
}
...
function activarMandoCinta(caja){
  ...
  function tomarControl(){
    ...
    btnPausa.setAttribute("aria-label", "Resume motion");
  }
  ...
  function soltarControl(){
    ...
    btnPausa.setAttribute("aria-label", "Pause motion");
  }
...
const Visor = {
  ...
  crear(){
    ...
    d.innerHTML =
      '<div class="visor-barra">' +
        ...
        '<button type="button" class="visor-cerrar" id="visor-cerrar" aria-label="Close">&times;</button>' +
      '</div>' +
      '<div class="visor-marco">' +
        '<button type="button" class="visor-flecha visor-izq" id="visor-izq" aria-label="Previous image">' + ICONOS.izq + '</button>' +
        '<img id="visor-img" src="" alt="">' +
        '<button type="button" class="visor-flecha visor-der" id="visor-der" aria-label="Next image">' + ICONOS.der + '</button>' +
      '</div>' +
  ...
  abrir(fotos, titulo, desde){
    ...
    $("#visor-pie").innerHTML = fotos.map((f, n) =>
      '<button type="button" data-visor="' + n + '" aria-label="View image ' + (n + 1) + '">' +
...
function tarjeta(p){
  ...
  const puntos = paleta && paleta.colores.length
    ? '<div class="tarjeta-colores" aria-label="' + paleta.colores.length + ' colors available">' +
  ...
  return '<article class="tarjeta">' +
    ...
    '<div class="tarjeta-foto" data-carrusel>' +
      ...
      (p.destacado ? '<span class="marcador">Best seller</span>' : '') +
      '<button type="button" class="tarjeta-flecha tarjeta-izq" data-paso="-1" aria-label="Previous photo">' + ICONOS.izq + '</button>' +
      '<button type="button" class="tarjeta-flecha tarjeta-der" data-paso="1" aria-label="Next photo">' + ICONOS.der + '</button>' +
    ...
    '<div class="tarjeta-cuerpo">' +
      ...
      '<div class="tarjeta-pie">' +
        ...
        '<button type="button" class="btn btn-chico ' + (dentro ? 'btn-oscuro' : 'btn-linea') + '" data-agregar="' + p.id + '">' +
          (dentro ? ICONOS.check + 'Added' : ICONOS.mas + 'Quote') + '</button>' +
      '</div>' +
    '</div></article>';
}
...
function bloqueMedio(item, carpeta){
  return '<figure class="medio">' +
    imagenFlexible("../img/" + carpeta + "/" + item.id,
                   item.nombre + " on a Serrania garment", "", "medio-sin-foto", "") +
